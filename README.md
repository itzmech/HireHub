...
- **Initial admin + demo data** — on a fresh Vercel deployment the SQLite database starts empty. Set `ADMIN_EMAIL` and `ADMIN_PASSWORD` as environment variables to have the initial admin account created automatically on the first cold boot (idempotent — the account is only created if the email is not already present). Optionally set `SEED_DEMO_JOBS=1` to populate the jobs table on an empty database. Local development is unaffected (`npm run seed`).
