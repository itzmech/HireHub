...
- **Initial admin + demo data** — every fresh serverless database is seeded automatically on the first cold boot: a demo admin (**`admin@demo.com`** / **`DemoAdmin@123`** — public demo credentials for this capstone project) and six sample job listings if the jobs table is empty. Override the credentials with `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_NAME`, or skip admin seeding entirely with `DISABLE_DEMO_ADMIN=1`; `SEED_DEMO_JOBS=0` skips the sample listings. Seeding is idempotent — existing accounts are never duplicated or promoted. Local development is unaffected (`npm run seed`). Note: on Vercel the SQLite database is ephemeral (per cold start), so seeded data only survives within a warm instance — durable production data requires the hosted-DB swap described below.
